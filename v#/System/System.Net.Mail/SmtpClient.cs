...
namespace System.Net.Mail {
	...
	//	Mutex mutex = new Mutex ();
		...
		public SmtpClient ()
		{
			this.with_host_port(null, 0);
		}

		public SmtpClient.with_host(string host)
		{
			this.with_host_port(host, 0);
		}

		public SmtpClient.with_host_port (string host, int port) {
...
// TODO //
		public signal SendCompletedEventHandler SendCompleted();
...
*/
		...
		{
			string output = GenerateBoundary_WithIndex (boundaryIndex);
			boundaryIndex += 1;
			return output;
		}

		private static string GenerateBoundary_WithIndex (int index)
		{
...
//	TODO		} catch (SmtpException) {
			} catch (Error e) {
				throw e;
//	TODO		} catch (Exception ex) {
			} catch (Error e) {
//	TODO			throw new SmtpException ("Message could not be sent.", ex);
				throw e;
			} finally {
		...
		private void SendCore (MailMessage message)
		{
			...
			status = Read ();
			if (IsError (status))
//	TODO			throw new SmtpException (status.StatusCode, status.Description);
